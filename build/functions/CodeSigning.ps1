...
function CodeSigning.SignExecutable {
    ...
    $process = Start-Process "C:/Program Files (x86)/Microsoft SDKs/ClickOnce/SignTool/signtool.exe" "sign /f mulderload.pfx /p $Env:CERTIFICATE_PASSWORD /t http://timestamp.digicert.com /fd sha256 $exePath" -PassThru -Wait
